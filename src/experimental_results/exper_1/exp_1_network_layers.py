...
def variable_summaries(var, name):
    ...
    :param name: string, name of the variable we want to summarize
    :return: none
    """
    # Summaires must run on the CPU
...
def batch_normalization_wrapper(inputs, ten_shape, is_training, decay=.999, epsilon=.0000000001, on_cpu=True, gpu=0):
    """
    DESCRIPTION
        ...
        if batch_norm:
            with tf.variable_scope('batch_norm'):
                pre_activ = batch_normalization_wrapper(tf.nn.bias_add(drop_out, biases, name='add_biases_op'),
                                                        bias_shape, is_training, decay=.999, epsilon=.0000000001,
