...
import experimental_results.exper_2.exp_2_model as model
#import experimental_results.exper_1.exp_1_model as model
    ...
    with tf.Graph().as_default():
        ...
        with tf.device('/gpu:0'):
            with tf.name_scope('global_stepping'):
                ...
                global_step = tf.get_variable('global_step', [], initializer=tf.constant_initializer(0), trainable=False, dtype=tf.float32)
        with tf.device('/gpu:0'):
        ...
        with tf.device('/gpu:0'):
            with tf.name_scope('optimizer'):
                ...
                #opt_adam = tf.train.MomentumOptimizer(learning_rate=decay_learn_rate, momentum=0.9)
                opt_adam = tf.train.GradientDescentOptimizer(learning_rate=decay_learn_rate)
                gvs = opt_adam.compute_gradients(total_loss, colocate_gradients_with_ops=True)
                #capped_gvs = [(tf.clip_by_value(grad, -1., 1.), var) for grad, var in gvs]
                optimizer = opt_adam.apply_gradients(gvs, global_step=global_step)
                    ...
                    for decay_steps in decay_steps_list:
                        ...
                        if sub_count>0:
                            ...
                            for i in range(len(train_filenames_list)):
                                ...
                                #finished_first_epoch_event = True
                                #finished_training_event = False
                                #acc_not_improve_event = False

                                # Pull out the number of training examples
                                n_examples_per_epoch = train_numbexamples_list[i]
                                training_session(False, data_description, finished_first_epoch_event,
                                ...
                                #                                                  test_filenames,
                                #                                                  n_examples_per_epoch, n_classes, batch_size,
                                #                                                  n_epochs,
                                #                                                  batch_norm,
                                #                                                  regulizer, keep_prob, learning_rate,
                                #                                                  decay_rate, decay_steps))
