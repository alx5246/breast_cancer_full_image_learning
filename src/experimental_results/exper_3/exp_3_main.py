...
def training_session(is_restart, data_description_txt_file, finished_first_epoch_event, finished_training_event,
                     performance_drop_event, save_path, train_filenames, n_examples_per_epoch, n_classes, batch_size,
                     n_epochs, batch_norm, regulizer, keep_prob, learning_rate, decay_rate, decay_steps):
    """
    DESCRIPTION
    ...
    if not is_restart:
        # We need to make a new directory for our experiment, thus we need to see what directories already exist! To
        # do this we are going to find out how many directories already exist, and then make a new one!
    ...
    exp_dir = os.path.join(save_path, '_exper_3_' + str(experiment_numb).zfill(4))
    ...
    with tf.Graph().as_default():
        ...
        with open(run_log_text_path, "a") as log_file:
            log_file.write("\nGenerate decaying learning rate")

        with tf.device('/gpu:0'):
            with tf.name_scope('decaying_learning_rate'):
                # You have to give the decay-rate the 'global_step' you want to use to control decay (pointer to the
                # variable essentially) or this will decay every time it is called I think.
        ...
        with tf.device('/gpu:0'):
            with tf.name_scope('optimizer'):
                ...
                opt_optimizer = tf.train.MomentumOptimizer(learning_rate=decay_learn_rate, momentum=0.9,
                                                           name='SGD_Momentum_Optimizer')
                gvs = opt_optimizer.compute_gradients(total_loss, colocate_gradients_with_ops=True)
                capped_gvs = [(tf.clip_by_value(grad, -1., 1.), var) for grad, var in gvs]
                # I do not think I want to pass the optimzier the 'global_step' because it may auto-increment it.
                optimizer = opt_optimizer.apply_gradients(capped_gvs)
                ...
                if not performance_drop_event.is_set():
                    ...
                    try:
                        ...
                        for j in range(num_training_batches):
                            ...
                            #print('\n   Batch Run-Time = ' + str(time.time() - batch_start_time))

                            # Every so often, we want to output updates of the training procedure
                            actual_step_number = (sess.run(global_step)) * num_training_batches + j + 1
                            if actual_step_number % 50 == 0:
                                summary_str = sess.run(summary_op)
                                summary_writer.add_summary(summary_str, actual_step_number)
                            ...
                            #     the_file_name = os.path.join(exp_dir, 'timeline.json')
                        ...
                        with open(run_log_text_path, "a") as log_file:
                            log_file.write('\n\nERROR ERROR, oops, something went wrong in trying to run training-epoch'
                                           ', try-except failed out at global-step ' + str(sess.run(global_step)) +
                                           ' \n')

                        with open(fail_log_test_path, "a") as fail_log:
                            fail_log.write('\nERROR ERROR, oops, something went wrong in trying to run training-epoch,'
                                           ' try-except at global-step: '
                                           + str(sess.run(global_step) + " at time " + str(datetime.now())))

                        print("\nTRAINING: SOMETHING IN TRAINING-EPOCH WENT WRONG!!, WILL TRY AGAIN, B"
...
def evaluation_session(is_restart, data_description_txt_file, finished_training_event, performance_drop_event,
                       ...
                       batch_size, n_epochs, batch_norm, regulizer, keep_prob, learning_rate, decay_rate, decay_steps):
                    ...
                    for decay_steps in decay_steps_list:
                        ...
                        if sub_count>0:
                            ...
                            for i in range(len(train_filenames_list)):
                                ...
                                # Create the Multi-Processing events that will handle when the training and the testing
                                # threadseach begin
                                ...
                                # Start the training process over a seperate thread.
                                p1 = mp.Process(target=training_session, args=(False, data_description,
                                                                               finished_first_epoch_event,
                                                                               finished_training_event,
                                                                               acc_not_improve_event,
                                                                               save_path, train_filenames,
                                                                               n_examples_per_epoch, n_classes,
                                                                               batch_size_training, n_epochs, batch_norm,
                                                                               regulizer, keep_prob, learning_rate,
                                                                               decay_rate, decay_steps))
                                p1.start()

                                # Wait until the Multi-Processing event is set, this occurs only after the first
                                # training iteration is complete, and is necessary so that the testing evaluation
                                ...
                                # Start the testing
                                p2 = mp.Process(target=evaluation_session, args=(False, data_description,
                                                                                 finished_training_event,
                                                                                 acc_not_improve_event, 20, save_path,
                                                                                 test_filenames, n_examples_per_epoch,
                                                                                 n_classes, batch_size_testing, n_epochs,
                                                                                 batch_norm, regulizer, keep_prob,
                                ...
                                while not finished_training_event.is_set() and not acc_not_improve_event.is_set():
                                    ...
                                    if not finished_training_event.is_set() and not acc_not_improve_event.is_set() and not p1.is_alive():
                                        ...
                                        p1 = mp.Process(target=training_session,
                                                        args=(True, data_description, finished_first_epoch_event,
                                                              ...
                                                              n_classes, batch_size_training, n_epochs, batch_norm,
                                                              regulizer, keep_prob, learning_rate, decay_rate, decay_steps))
                                        p1.start()
                                    ...
                                    if not finished_training_event.is_set() and not acc_not_improve_event.is_set() and not p2.is_alive():
                                        n_examples_per_epoch = test_numbexamples_list[i]
                                        # Start the testing
                                        p2 = mp.Process(target=evaluation_session,
                                                        args=(True, data_description, finished_training_event,
                                                              acc_not_improve_event, 20, save_path,
                                                              test_filenames,
                                                              n_examples_per_epoch, n_classes,
                                                              batch_size_testing, n_epochs,
                                                              batch_norm,
                                                              regulizer, keep_prob, learning_rate, decay_rate, decay_steps))
                                        p2.start()
