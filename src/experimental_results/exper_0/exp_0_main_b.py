...
if __name__ == "__main__":
    ...
    n_epochs = 250
    batch_norm = True
    regulizer_list = [0.0, .01, .1, 1.0]
    keep_prob_list = [1.0, .9, .8, .7, .6]
    learning_rate = .001
        ...
        for regulizer in regulizer_list:
            ...
            for keep_prob in keep_prob_list:
                ...
                for i in range(len(train_filenames_list)):
                    ...
                    data_description = data_description_list[i]
                    ...
                    p1 = mp.Process(target=training_session, args=(data_description, finished_first_epoch_event,
                                                                   finished_training_event, save_path,
                                                                   train_filenames, n_examples_per_epoch, n_classes,
                                                                   batch_size,
                                                                   n_epochs, batch_norm, regulizer, keep_prob, learning_rate))
                    p1.start()
                    # Wait until the Multi-Processing event is set
