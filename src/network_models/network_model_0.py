...
def generate_res_network(images, batch_size, n_classes, batch_norm=True, is_training=True):
    ...
    for i in range(layer_1_num_res):
        ...
        with tf.variable_scope(var_name):
            net_layers.append(nl.res_block(net_layers[-1], layer_1_depth, down_sample=False, batch_norm=True, is_training=True))
    ...
    for i in range(layer_2_num_res):
        ...
        with tf.variable_scope(var_name):
            if i == 0:
                net_layers.append(
                    nl.res_block(net_layers[-1], layer_2_depth, down_sample=True, batch_norm=True, is_training=True))
            else:
                net_layers.append(
                    nl.res_block(net_layers[-1], layer_2_depth, down_sample=False, batch_norm=True, is_training=True))
    ...
    for i in range(layer_3_num_res):
        ...
        with tf.variable_scope(var_name):
            if i == 0:
                net_layers.append(
                    nl.res_block(net_layers[-1], layer_3_depth, down_sample=True, batch_norm=True, is_training=True))
            else:
                net_layers.append(
                    nl.res_block(net_layers[-1], layer_3_depth, down_sample=False, batch_norm=True, is_training=True))
    ...
    for i in range(layer_4_num_res):
        ...
        with tf.variable_scope(var_name):
            if i == 0:
                net_layers.append(
                    nl.res_block(net_layers[-1], layer_4_depth, down_sample=True, batch_norm=True, is_training=True))
            else:
                net_layers.append(
                    nl.res_block(net_layers[-1], layer_4_depth, down_sample=False, batch_norm=True, is_training=True))
    ...
    for i in range(layer_5_num_res):
        ...
        with tf.variable_scope(var_name):
            if i == 0:
                net_layers.append(
                    nl.res_block(net_layers[-1], layer_5_depth, down_sample=True, batch_norm=True,
                                  is_training=True))
            else:
                net_layers.append(
                    nl.res_block(net_layers[-1], layer_5_depth, down_sample=False, batch_norm=True,
                                  is_training=True))
    ...
    with tf.variable_scope('sixth_layer_max_pool'):
        net_layers.append(tf.nn.max_pool(net_layers[-1], ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='max_pool_op'))

    # Input at this point should be [n, 4, 4, 1024]
    # apply a max pool operation
    with tf.variable_scope('seventh_layer_avg_pool'):
        net_layers.append(tf.nn.avg_pool(net_layers[-1], ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='max_pool_op'))
